...
object DeviceUtils {
    ...
    fun getDefaultLoadFactor(context: Context): Int {
        if (_loadFactor == null) {
            ...
            _loadFactor = integer
            _loadFactor =
                Integer.valueOf(Math.max(integer.toInt(), 1))
        }
        return _loadFactor!!.toInt()
    }

    fun getDensity(context: Context): Float {
        if (displayDensity.toDouble() == 0.0) displayDensity =
            getDisplayMetrics(context).density
    ...
    fun hasBigScreen(context: Context): Boolean {
        ...
        if (_hasBigScreen == null) {
            val flag1: Boolean
            flag1 = if (0xf and context.resources
    ...
    fun hasCamera(context: Context): Boolean {
        if (_hasCamera == null) {
            ...
            val flag2: Boolean
            flag2 = if (flag || flag1) true else false
            _hasCamera = java.lang.Boolean.valueOf(flag2)
        }
        return _hasCamera?:false
    }
    ...
    fun hasHardwareMenuKey(context: Context?): Boolean {
        var flag = false
        flag = if (PRE_HC) true else if (GTE_ICS) {
    ...
    fun hasInternet(context: Context): Boolean {
        ...
        flag = if (manager != null && manager.activeNetworkInfo != null) true else false
        return flag
    }
    ...
    fun hideAnimatedView(view: View?) {
        if (PRE_HC && view != null) view.setPadding(view.width, 0, 0, 0)
    }
    ...
    fun isLandscape(context: Context): Boolean {
        val flag: Boolean
        flag = if (context.resources.configuration.orientation == 2) true else false
        return flag
    }
    ...
    fun isPortrait(context: Context): Boolean {
        var flag = true
        if (context.resources.configuration.orientation != 1) flag = false
        return flag
    }
    ...
    fun isTablet(context: Context): Boolean {
        if (_isTablet == null) {
            val flag: Boolean
            flag = if (0xf and context.resources
                    .configuration.screenLayout >= 3
            ) true else false
    ...
    fun showAnimatedView(view: View?) {
        if (PRE_HC && view != null) view.setPadding(0, 0, 0, 0)
    }
